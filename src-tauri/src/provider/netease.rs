...
/// test
pub fn test(params : NeteaseParam) -> Result<(), Box<dyn Error>> {
    let s = params.order.unwrap_or_else(|| String::from("order is None"));
    Ok(())
}

/// 获取自定义专辑列表
pub fn ne_custom_album_list(params : NeteaseParam) -> Result<Vec<CustomAlbum>, Box<dyn Error>> {
    ...
    let resp = reqwest::blocking::get(uri).unwrap_or_else(| err | {
        panic!("ne_custom_album_list error is {}", err);
    });
    let text = resp.text()?;
    ...
    for li_element in fragment.select(&li_selector) {
        ...
        let source_url = format!("https://music.163.com/#/playlist?id={}", id);
